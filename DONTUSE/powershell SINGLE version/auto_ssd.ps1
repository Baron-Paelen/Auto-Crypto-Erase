...
$pw = Gen-PW
$blres = Enable-BitLocker -MountPoint $DriveLetter -EncryptionMethod Aes256 -PasswordProtector -Password $(ConvertTo-SecureString -String "$pw" -AsPlainText -Force) #-UsedSpaceOnly
if ($blres) {
...
$encrypted = $false
while (-not $encrypted) {
    $info = Get-BitLockerVolume -MountPoint $DriveLetter -ErrorAction SilentlyContinue

    if ($info.VolumeStatus -eq "FullyEncrypted") {
        $encrypted = $true
    } else {
        $percent = $info.EncryptionPercentage
        Write-Host "`nDrive $DriveLetter encryption at $percent%..." -ForegroundColor Yellow
        Start-Sleep -Seconds 20
    }
}

Write-Host "`nDrive $DriveLetter finished encrypting!" -ForegroundColor Green
